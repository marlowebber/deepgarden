...
const Color color_offwhite          = Color( 1.0f, 1.0f, 0.8f, 1.0f );
...
std::string exampleAnimal = std::string(" ddna gmmmam ");
...
// #define ORGAN_VACUOLE 32 // an empty organ that is removed at the end of development to leave behind shaped voids.
	...
	for (unsigned int i = 0; i < squareSizeAnimalSprite; ++i)
	{
		...
		// if      (a->segments[segmentNumber].frames[ frameOffset + i] == ORGAN_OUTSIDECONNECTION)
		// {
		// 	printf("_");
		...
		switch (a->segments[segmentNumber].frames[ frameOffset + i])
		{
		case ORGAN_NOTHING:
		{
			printf("_");
...
// 		)
// 		{
// 			a->segments[segmentNumber].frames[frameOffset + i] = ORGAN_OUTSIDECONNECTION;						// set the alpha to -1.0f, which does not occur in our drawings normally, and so can be used to mark a pixel with connection to the edge.
// 		}
// 	}
...
// 				if (
// 				    a->segments[segmentNumber].frames[ frameOffset +  neighbour ] == ORGAN_OUTSIDECONNECTION
// 				)
// 				{
// 					a->segments[segmentNumber].frames[ frameOffset +  i ] = ORGAN_OUTSIDECONNECTION;
// 				}
// 			}
// 		}
// 	}
...
// 	{
// #ifdef ANIMAL_DRAWING_READOUT
// 		printf("Add a new segment. : ");
// #endif

// 		if (animalCursorSegmentNumber < (maxAnimalSegments - 1))
// 		{
// 			animals[animalIndex].segmentsUsed++;
// #ifdef ANIMAL_DRAWING_READOUT
// 			printf("Drawing in a new segment was permitted.\n");
// #endif
// 		}
// 		else
// 		{
// #ifdef ANIMAL_DRAWING_READOUT
// 			printf("The maximum number of segments is reached.\n");
// #endif
// 		}
		...
		for (unsigned int j = animalCursorSegmentNumber; j < limit; ++j)
		{
			for (unsigned int frame = 0; frame < numberOfFrames; ++frame)
			{
				...
				for (unsigned int k = 0; k < squareSizeAnimalSprite; ++k)
				{
					...
					)
					{
						...
						if ( nSameOrganNeighours > 0)
						{
							if (animalCursorExtrusionCondition == CONDITION_GREATERTHAN)
							{
								...
								{
									animals[animalIndex].segments[j].frames[ (squareSizeAnimalSprite * frame ) + k ] = ORGAN_MARKED_A;
								}
							}
							...
							{
								// NOTE: It can still be less than, but it cannot grow at zero. that's cheating.
								if ( nSameOrganNeighours < animalCursorExtrusionN)
								{
									animals[animalIndex].segments[j].frames[ (squareSizeAnimalSprite * frame ) + k ] = ORGAN_MARKED_A;
								}
					...
					for (unsigned int k = 0; k < squareSizeAnimalSprite; ++k)
					{
						if (animals[animalIndex].segments[j].frames[ (squareSizeAnimalSprite * frame ) + k ] == ORGAN_MARKED_A)
						{
							animals[animalIndex].segments[j].frames[ (squareSizeAnimalSprite * frame ) + k ] = animalCursorOrgan;
						}
					}
		...
		case 'w':
		{
			...
			for (unsigned int j = animalCursorSegmentNumber; j < limit; ++j)
			{
				...
				for (unsigned int k = 0; k < squareSizeAnimalSprite; ++k)
				{
					...
					)
					{
						...
						if ( nSameOrganNeighours > 0)
						{
							if (animalCursorExtrusionCondition == CONDITION_GREATERTHAN)
							{
								if ( nSameOrganNeighours > animalCursorExtrusionN)
								{
									animals[animalIndex].segments[j].frames[ (squareSizeAnimalSprite * animalCursorFrame ) + k ] = ORGAN_MARKED_A;
								}
							}
							else if (animalCursorExtrusionCondition == CONDITION_EQUAL)
							{
					...
					for (unsigned int k = 0; k < squareSizeAnimalSprite; ++k)
					{
						if (animals[animalIndex].segments[j].frames[ (squareSizeAnimalSprite * animalCursorFrame ) + k ] == ORGAN_MARKED_A)
						{
							animals[animalIndex].segments[j].frames[ (squareSizeAnimalSprite * animalCursorFrame ) + k ] = animalCursorOrgan;
						}
					}
...
// 			unsigned int pixelOffset = (squareSizeAnimalSprite * FRAME_B) + k;
...
// 		for (unsigned int k = 0; k < (squareSizeAnimalSprite); ++k)
// 		{
// 			unsigned int pixelIndexA = (squareSizeAnimalSprite * FRAME_A) + k;
// 			unsigned int pixelIndexB = (squareSizeAnimalSprite * FRAME_B) + k;
// 			unsigned int pixelIndexC = (squareSizeAnimalSprite * FRAME_C) + k;
...
// 				for (unsigned int l = 0; l < 4; ++l)
// 				{
// 					unsigned int neighbourIndex = pixelIndexA + (neighbours[l] % (squareSizeAnimalSprite));
...
// 	else
// 	{
// 		a.fluidMovementChance = 16 - ( avgAreaDiff / 8 );
// 	}
...
// 				for (unsigned int l = 0; l < 4; ++l)
// 				{
// 					unsigned int neighbourIndex = pixelIndexB + (neighbours[l] % (squareSizeAnimalSprite));
...
// 	animals[animalIndex] = a;
	...
	void clearAnimalSpritePixel ( AnimalSegment * s, unsigned int pixelIndex )
	{
		...
		if ( (s->frames[(squareSizeAnimalSprite * s->animationFrame) + pixelIndex]).a != ORGAN_NOTHING  ) // only clear the pixel if it is not empty in the original image. This prevents disturbing other sprites.
		{
...
// travel from the indicated square in the light direction, marking cells as illuminated or dark along your way.
	...
	void setAnimalSpritePixel ( AnimalSegment * s, unsigned int pixelIndex )
	{
		...
		if ( worldI > totalSize) {return;}
		int j__color_offset = (worldI * numberOfFieldsPerVertex) ;
		// memcpy( &animationGrid[ j__color_offset], 	&( s.frames[(squareSizeAnimalSprite * s.animationFrame) + pixelIndex]  ) , 	sizeof(Color) );

		// if (
		// (s->frames[(squareSizeAnimalSprite * s->animationFrame) + pixelIndex]  != ORGAN_NOTHING )
		//  ) // only clear the pixel if it is not empty in the original image. This prevents disturbing other sprites.
		// {
		Color organColor = organColorLookup(  s->frames[(squareSizeAnimalSprite * s->animationFrame) + pixelIndex]  );
...
// unsigned int animalFall (unsigned int i)
// {
// 	unsigned int currentPosition = i;
// 	unsigned int animalIndex = seedGrid[i].parentIdentity;
...
// 		{
// 			currentPosition = ((currentPosition - sizeX) + (extremelyFastNumberFromZeroTo(2) - 1 )) % totalSize;
		...
		for (unsigned int segmentIndex = 0; segmentIndex < animals[animalIndex].segmentsUsed; ++segmentIndex)
		{
			for (unsigned int j = 0; j < numberOfFairTurnsPerSegment; ++j)
			{
				...
				if (!moved)
				{
					...
					// unsigned int pixelIndexC = (squareSizeAnimalSprite * FRAME_C) + spriteRandomI;
					bool pixelStateA = false;
					bool pixelStateB = false;
					// bool pixelStateC = false;

					if (animals[animalIndex].segments[segmentIndex].frames[pixelIndexA].a > 0.0f ) {pixelStateA = true;}
					if (animals[animalIndex].segments[segmentIndex].frames[pixelIndexB].a > 0.0f ) {pixelStateB = true;}
					// if (animals[animalIndex].segments[segmentIndex].frames[pixelIndexC].a > 0.0f ) {pixelStateC = true;}

					if (  (pixelStateA != pixelStateB ) && (!(animals[animalIndex].moved)) ) // this position can be moved into if you like.
					{
